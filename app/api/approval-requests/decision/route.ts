...
export async function POST(request: Request) {
  ...
  if (account?.auth_user_id) await createPortalTask(access.admin, { businessId: access.profile.business_id, recipientUserId: account.auth_user_id, recipientRole: "employee", title: "Request updated", message: `Your ${String(approval.request_type || "approval").toLowerCase()} was ${status.toLowerCase()}.`, href: "/employee/tasks", taskType: "approval_decision" });
  await sendOneSignalPush({ externalIds: account?.auth_user_id ? [account.auth_user_id] : [], title: "Request updated", message: `Your ${String(approval.request_type || "approval").toLowerCase()} was ${status.toLowerCase()}.`, url: `${new URL(request.url).origin}/employee/tasks` });
