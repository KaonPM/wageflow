...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  const access = await requireRole(request, ["employer", "employee", "master", "master_admin"]);
  ...
  const { data, error } = await access.admin.storage.from("employee-documents").createSignedUrl(path, 60);
